#!/usr/bin/env python3
"""
generate_jl_notebook.py
Produces content/PRISM_VPP_Demo.ipynb compatible with JupyterLite / Pyodide.
Differences from the Colab version:
  - micropip installs instead of pip subprocess
  - no google.colab imports
  - no display(Javascript(...))
  - no file-system writes (pathlib OUTPUT)
  - all BENCH data, chart code, HTML displays identical
"""
...
def code(src):
    return {"cell_type":"code","id":uid(),"metadata":{},"execution_count":None,
            "outputs":[],"source":src}
...
# ── TITLE ────────────────────────────────────────────────────────────────────
cells.append(md(r'''# PRISM VPP Dispatch Demo
...
# ── SETUP — micropip (JupyterLite/Pyodide) ───────────────────────────────────
cells.append(code(
r'''# Install packages via micropip (runs in-browser via Pyodide)
import sys
...
_CSS = (
    '<style>'
    ':root{--bg:#f6f8fa;--card:#fff;--card2:#f6f8fa;--bdr:#d0d7de;'
    '--tx:#1f2328;--tx2:#57606a;--acc:#0969da;--grn:#1a7f37;--amb:#9a6700;--red:#d1242f;}'
    '.pcard{background:var(--card);border:1px solid var(--bdr);border-radius:8px;'
    'padding:20px 24px;margin:12px 0;font-family:-apple-system,sans-serif;color:var(--tx);'
    'box-shadow:0 1px 3px rgba(0,0,0,.06);}'
    '.phdr{background:linear-gradient(135deg,#eef4ff,#f0f6ff);'
    'border:1px solid #bfdbfe;border-top:3px solid var(--acc);'
    'border-radius:8px;padding:28px 36px;margin:16px 0;font-family:-apple-system,sans-serif;}'
    '.bdg{display:inline-block;padding:3px 10px;border-radius:20px;font-size:10.5px;'
    'font-weight:600;letter-spacing:.04em;font-family:monospace;}'
    '.bdg-live{background:#dcfce7;color:#15803d;border:1px solid #86efac;}'
    '.bdg-cached{background:#dbeafe;color:#1d4ed8;border:1px solid #93c5fd;}'
    '.bdg-synth{background:#ede9fe;color:#7c3aed;border:1px solid #c4b5fd;}'
    '.bdg-assum{background:#fef9c3;color:#854d0e;border:1px solid #fde047;}'
    '.mgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin:12px 0;}'
    '.mbox{background:var(--card2);border:1px solid var(--bdr);border-radius:6px;padding:14px;text-align:center;}'
    '.mval{font-size:19px;font-weight:700;color:var(--acc);font-family:monospace;line-height:1.2;}'
    '.mlbl{font-size:10px;color:var(--tx2);margin-top:4px;text-transform:uppercase;letter-spacing:.05em;}'
    '.stitle{font-size:12px;font-weight:700;color:var(--tx2);text-transform:uppercase;'
    'letter-spacing:.1em;border-bottom:1px solid var(--bdr);padding-bottom:8px;margin-bottom:14px;}'
    '.ipnote{background:#f0f6ff;border:1px solid #bfdbfe;border-left:3px solid var(--acc);'
    'border-radius:4px;padding:12px 16px;font-size:12px;color:#57606a;font-family:monospace;margin:12px 0;}'
    '.ptbl{width:100%;border-collapse:collapse;font-family:monospace;font-size:12.5px;}'
    '.ptbl th{background:var(--card2);border:1px solid var(--bdr);padding:8px 12px;text-align:left;'
    'color:var(--tx2);font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;}'
    '.ptbl td{border:1px solid #e9ecef;padding:7px 12px;color:var(--tx);}'
    '.ptbl tr:hover td{background:var(--card2);}'
    '.grn{color:#1a7f37!important;}.amb{color:#9a6700!important;}'
    '.red{color:#d1242f!important;}.blu{color:#0969da!important;}'
    '</style>'
)
...
# ── BENCH DATA ────────────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Validated cached benchmark data ─────────────────────────────────────────
BENCH = {
    "gpu_scale": dict(
...
# ── FLEET GENERATOR ────────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 1: Synthetic Fleet Dataset ───────────────────────────────────────
display(HTML(
...
# ── MARKET WINDOW ─────────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 2: 5-Minute Market Clearing Window ───────────────────────────────
h = np.arange(T)
...
cells.append(code(
r'''# ── Section 3: Coordination Premium — THE KEY RESULT ─────────────────────────
display(HTML(
...
# ── SOLVER COMPARISON ──────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 4: Solver Comparison  [CACHED] ────────────────────────────────────
...
# ── INTERACTIVE SLIDERS ───────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 5: Interactive — Adjust Parameters ───────────────────────────────
display(HTML(
...
# ── ECONOMICS ─────────────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 6: Business Case [CACHED + ASSUMPTION] ───────────────────────────
scenarios = [
...
# ── WARM START ────────────────────────────────────────────────────────────────
cells.append(code(
r'''# ── Section 7: Warm-Start at 500k Units  [CACHED] ────────────────────────────
ws = BENCH["warmstart"]
display(HTML(
